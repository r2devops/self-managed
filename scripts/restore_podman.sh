...
if podman run --replace --name postgres-bck --mount=type=bind,source=$BACKUP_DIR,destination=/backup -e PGPASSWORD=$JOBS_DB_PASSWORD --network intranet postgres:$PG_VERSION /bin/bash -c "pg_restore -U $JOBS_DB_USER -h $JOBS_DB_HOST --if-exists -c -e -Ft -d $JOBS_DB_NAME /backup/db_backup.tar"; then
